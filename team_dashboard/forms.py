...
class PostTrainingForm(forms.Form):
    date= DateTimeField()
    type_of_activity= TrainingField(required=False)
    time_of_activity= forms.DecimalField(widget=forms.TextInput(attrs={'class': 'form-control'}))
    perceived_strain_of_activity= forms.IntegerField(widget=StarRating(attrs={'class':'star-rating'}, max_stars=10, tags=['Max. Esfuerzo', 'Moderado', 'Leve']), required=True)
